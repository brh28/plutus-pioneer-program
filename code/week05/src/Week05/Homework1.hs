...
mkPolicy pkh deadline ctx = traceIfFalse "deadline has passed" (beforeDeadline `contains` txInfoValidRange txInfo) 
                            && traceIfFalse "must be signed by pubkey" (txSignedBy txInfo pkh)
    where
        txInfo :: TxInfo
        txInfo = scriptContextTxInfo ctx 

        beforeDeadline :: SlotRange
        beforeDeadline = to deadline
